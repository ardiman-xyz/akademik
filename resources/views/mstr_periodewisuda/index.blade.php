...
@section('pageTitle', 'Periode Wisuda')
@section('content')
...
<section class="content">
  ...
  <div class="container-fluid-title">
    <div class="title-laporan">
      <h3 class="text-white">Periode wisuda</h3>
    </div>
  </div>
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      <div class="panel-heading-green">
        <div class="pull-right tombol-gandeng dua">
          ...
          @if(in_array('periodewisuda-CanAdd', $acc))<a href="{{ url('proses/periodewisuda/create?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Tambah data &nbsp;<i class="fa fa-plus"></i></a>@endif
        </div>
        <b>Periode Wisuda</b>
      </div>
    </div>
    <br>
        <div class="bootstrap-admin-box-title right text-white">
          <!-- <b>Daftar Fakultas</b> -->
          {!! Form::open(['url' => route('periodewisuda.index') , 'method' => 'GET', 'name' => 'form', 'class' => 'row text-green', 'style' => 'padding-top:5px;padding-left:15px;padding-right:15px;', 'role' => 'form']) !!}
        ...
        <table class="table table-striped table-font-sm">
          <thead class="thead-default thead-green">
              <tr>
                  ...
                  <th width="25%">Tanggal Wisuda</th>
                  <th width="25%">Tempat Wisuda</th>
                  <th width="20%">Tangggal terakhir Yudisium</th>
                  @if(in_array('periodewisuda-CanEdit', $acc) || in_array('periodewisuda-CanDelete', $acc))
          ...
          <tbody>
            ...
            foreach ($query as $data) {
              ...
              // if (count($prod)==0) {
              $date = strtotime($data->Graduation_Date);
              $Graduation_Date = date('Y-m-d', $date);
              $date2 = strtotime($data->End_Date_Yudisium);
              $End_Date_Yudisium = date('Y-m-d', $date2);
              ?>
              <tr>
                  ...
                  <td>{{ tanggal_indo($Graduation_Date,false)}}</td>
                  <td>{{ $data->Graduation_Place }}</td>
                  <td>{{ tanggal_indo($End_Date_Yudisium,false)}}</td>
                  @if(in_array('periodewisuda-CanEdit', $acc) || in_array('periodewisuda-CanDelete', $acc))
                  <td align="center">
                      {!! Form::open(['url' => route('periodewisuda.destroy', $data->Graduation_Period_Id) , 'method' => 'delete', 'role' => 'form']) !!}
                      {{-- <a href="{{ url('master/country/'.$data->Country_Id.'?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Detail</a> --}}
                      @if(in_array('periodewisuda-CanEdit', $acc))<a href="{{ url('proses/periodewisuda/'.$data->Graduation_Period_Id.'/edit?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-info btn-sm">Edit</a>@endif
                      @if(in_array('periodewisuda-CanDelete', $acc)){!! Form::button('Hapus', ['type'=>'submit','class'=>'btn btn-danger btn-sm hapus','data-id'=>$data->Graduation_Period_Id]) !!}@endif
                      {!! Form::close() !!}
                  </td>
                  @endif
              </tr>
  ...
  $(document).on('click', '.hapus', function (e) {
    ...
    //  console.log(id);
      ...
      if (isConfirm) {
              $.ajax({
                  url: "{{ url('') }}/proses/periodewisuda/" + id,
                  type: "DELETE",
                  dataType: "json",
                  data: {
                    "_token": "{{ csrf_token() }}"
