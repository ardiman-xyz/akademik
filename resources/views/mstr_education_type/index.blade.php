...
<section class="content">
  ...
  <div class="container-fluid-title">
    <div class="title-laporan">
      <h3 class="text-white">Jenjang Pendidikan</h3>
    </div>
  </div>
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      <div class="panel-heading-green">
        <div class="pull-right tombol-gandeng dua">
          ...
          <a href="{{ url('master/education_type/create?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Tambah data &nbsp;<i class="fa fa-plus"></i></a>
        </div>
      ...
      <br>
          <!-- <b>Daftar Jenjang Pendidikan</b> -->
          {!! Form::open(['url' => route('education_type.index') , 'method' => 'GET', 'name' => 'form', 'class' => 'row text-green', 'style' => 'padding-top:5px;padding-left:15px;padding-right:15px;', 'role' => 'form']) !!}
        ...
        <table class="table table-striped table-font-sm">
          <thead class="thead-default thead-green">
              <tr>
                  ...
                  <th width="5%">No</th>
                  <th width="40%">Kode Jenjang Pendidikan</th>
                  <th width="40%">Nama Jenjang Pendidikan</th>
          ...
          <tbody>
            ...
            foreach ($query as $data) {
              ...
              <tr>
                  <!-- <th></th> -->
                  <td>{{ $a }}</td>
                  <td>{{ $data->Education_Type_Code }}</td>
                  <td>{{ $data->Education_Type_Name }}</td>
                  <td align="center">
                      {!! Form::open(['url' => route('education_type.destroy', $data->Education_Type_Id) , 'method' => 'delete', 'role' => 'form']) !!}
                      <a href="{{ url('master/education_type/'.$data->Education_Type_Id.'/edit?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-info btn-sm">Edit</a>
                      {!! Form::button('Hapus', ['type'=>'submit','class'=>'btn btn-danger btn-sm hapus','data-id'=>$data->Education_Type_Id]) !!}
                      {!! Form::close() !!}
                  </td>
              </tr>
              <?php
  ...
  $(document).on('click', '.hapus', function (e) {
    ...
    //  console.log(id);
      ...
      if (isConfirm) {
              $.ajax({
                  url: "{{ url('') }}/master/education_type/" + id,
                  type: "DELETE",
                  dataType: "json",
                  data: {
                    "_token": "{{ csrf_token() }}"
