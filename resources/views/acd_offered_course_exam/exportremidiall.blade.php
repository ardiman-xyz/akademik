...
<body>
      ...
      <tr>
        ...
        <td width=" 70%"><center><b>
          ...
          if($exam_typee == 3){
            ...
            echo "PRESENSI REMIDI PROGRAM STUDI ".$member['data']->Acronym.' '.$member['data']->Department_Name.' SEMESTER '.$smt.' TAHUN AKADEMIK '.$th;
          }
          ?>
        </td>
        <td width=" 15%"></td>
    ...
    <tr>
      ...
      <td style="width:34%;">{{ $member['data']->Course_Name  }}</td>
      <td style="width:15%;">Kelas</td><td style="width:1%;">:</td>
      <td style="width:34%;">{{$member['data']->Class_Name}}</td>
    </tr>
    <tr>
      <td>Dosen Pengampu</td><td>:</td>
      <td><?php 
      $id_dosen = explode('|',$member['data']->id_dosen);
        $dsn_matkul = [];
        $x=0;
    ...
    <tr>
      ...
      <td>
        <?php
          $n = 0;
          if ($member['data']->Exam_Start_Date != "") {
            $key = $member['data']->Exam_Start_Date;
                $start = explode(" ",$key);
                $s_date = $start[0];
    ...
    <?php
        $lnm2 = strtolower($member['data']->Pengawas_1); $ucnm2 = ucwords($lnm2);
        $lnm3 = strtolower($member['data']->Pengawas_2); $ucnm3 = ucwords($lnm3);
      ?>
  </table>
  <br>
  ...
  <table class="tablex" border="1px" style="border-collapse : collapse; width:100%; font-size:13px;">
    ...
    <?php
    $no = 1;
    foreach ($member['mhs']  as $membera) {
      $totalpertemuan = DB::table('acd_sched_real')
        ->where('Course_Id',$member['data']->Course_Id)
        ->where('Term_Year_Id',$term_year)
        ->where('Class_Prog_Id',$class_prog)
        ->where('Class_Id',$member['data']->Class_Id)
        ->count();  
        ...
        $Register_Number = DB::table('acd_student')->where('Student_Id',$membera->Student_Id)->select('Register_Number')->first();
        ...
        if($exam_typee == 3){
          ...
          ->where('Student_Id',$membera->Student_Id)
          ->get();
          $totalbiayaremidi = 0;
          ...
          ->where('Register_Number',$membera->Register_Number)
          ->where('Is_Remediasi',1)
          ->get();
    ...
    ?>
     <tr>
      ...
      <td><center>{{ $membera->Nim }}</td>
      <?php $lnm = strtolower($membera->Full_Name); $ucnm = ucwords($lnm); 
        $t_p = 0;
        foreach ($totalpertemuans as $keys) {
            $total = DB::table('acd_sched_real_detail')
            ->where([['Sched_Real_Id',$keys->Sched_Real_Id],['Student_Id',$membera->Student_Id]])
            ->first();
            if($total != null){
                $t_p++;
            }
