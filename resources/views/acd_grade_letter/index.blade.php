...
<section class="content">
  ...
  <div class="container-fluid-title">
    <div class="title-laporan">
      <h3 class="text-white">Nilai Huruf</h3>
    </div>
  </div>
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      <div class="panel-heading-green">
        <div class="pull-right tombol-gandeng dua">
          ...
          <a href="{{ url('master/grade_letter/create?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Tambah data &nbsp;<i class="fa fa-plus"></i></a>
        </div>
      ...
      {{-- <br> --}}
          <!-- <b>Daftar Fakultas</b> -->
          {{-- {!! Form::open(['url' => route('grade_letter.index') , 'method' => 'GET', 'name' => 'form', 'class' => 'row text-green', 'style' => 'padding-top:5px;padding-left:15px;padding-right:15px;', 'role' => 'form']) !!}
          ...
          <tbody>
            ...
            foreach ($query as $data) {
              ...
              <tr>
                  <!-- <th></th> -->
                  <td>{{ $a }}</td>
                  <td><center>{{ $data->Grade_Letter }}</center></td>
                  <td align="center">
                      {!! Form::open(['url' => route('grade_letter.destroy', $data->Grade_Letter_Id) , 'method' => 'delete', 'role' => 'form']) !!}
                      <a href="{{ url('master/grade_letter/'.$data->Grade_Letter_Id.'/edit?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-info btn-sm">Edit</a>
                      {!! Form::button('Hapus', ['type'=>'submit','class'=>'btn btn-danger btn-sm hapus','data-id'=>$data->Grade_Letter_Id]) !!}
                      {!! Form::close() !!}
                  </td>
              </tr>
              <?php
  ...
  $(document).on('click', '.hapus', function (e) {
    ...
    //  console.log(id);
      ...
      if (isConfirm) {
              $.ajax({
                  url: "{{ url('') }}/master/grade_letter/" + id,
                  type: "DELETE",
                  dataType: "json",
                  data: {
                    "_token": "{{ csrf_token() }}"
