...
<section class="content">
  ...
  <div class="container-fluid-title">
    <div class="title-laporan">
      <h3 class="text-white">Mahasiswa Cuti</h3>
    </div>
  </div>
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      <div class="panel-heading-green">
        <div class="pull-right tombol-gandeng dua">
          <?php $page = ""; if(isset($_GET['page'])) { $page = $_GET['page']; }; ?>
            ...
            <a href="{{ url('proses/cuti/export/exportcuti?search='.$search.'&term_year='.$term_year) }}" class="btn btn-warning btn-sm">Export &nbsp;<i class="fa fa-print"></i></a>
      ...
      <br>
          <!-- <b>Daftar Fakultas</b> -->
          {!! Form::open(['url' => route('cuti.index') , 'method' => 'GET', 'name' => 'form', 'role' => 'form']) !!}
        ...
        <table id="myTable" class="table table-striped table-font-sm">
          <thead class="thead-default thead-green">
              <tr>
                ...
                <th width="8%">SK Date</th>
                <th width="8%">SK Number</th>
                <th width="8%">File</th>
          ...
          <tbody>
            ...
            foreach ($cuti as $data) {
              ...
              <tr>
                  ...
                  <!-- <td><center>{{ $data->Vacation_Reason }}</center></td> -->
                  <td><center>{{ $data->Description }}</center></td>
                  <td><center>{{ ($data->Is_Approved == 1 ? 'Ya':($data->Is_Approved == '0' ? 'Tidak':'')) }}</center></td>
                  <td><center>{{ $data->Sk_Date }}</center></td>
                  <td><center>{{ $data->Sk_Number }}</center></td>
                  <td><center><a href="{{route('getfile')}}?name={{$data->File}}" target="_blank">{{ $data->File }}</a> </center></td>
                  @if(in_array('cuti-CanDelete', $acc))
                  <td align="center">
                    {!! Form::open(['url' => route('cuti.destroy', $data->Student_Vacation_Id) , 'method' => 'delete', 'role' => 'form']) !!}
                  <a href="{{ url('proses/cuti/'.$data->Student_Vacation_Id.'/edit?&page='.$page.'&rowpage='.$rowpage.'&search='.$search.'&term_year='.$term_year) }}" class="btn btn-info btn-sm">Edit</a>
                    {!! Form::button('Hapus', ['type'=>'submit','class'=>'btn btn-danger btn-sm hapus','data-id'=>$data->Student_Vacation_Id]) !!}
                    {!! Form::close() !!}
                  </td>
                  @endif
              </tr>
