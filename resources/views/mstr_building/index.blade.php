  ...
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      <div class="panel-heading-green">
        <div class="pull-right tombol-gandeng dua">
          ...
          @if(in_array('building-CanAdd', $acc)) <a href="{{ url('master/building/create?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Tambah data &nbsp;<i class="fa fa-plus"></i></a>@endif
        </div>
        <b>Gedung</b>
      </div>
    </div>
    <br>
        <div class="bootstrap-admin-box-title right text-white">
          <!-- <b>Daftar Fakultas</b> -->
          {!! Form::open(['url' => route('building.index') , 'method' => 'GET', 'name' => 'form', 'class' => 'row text-green', 'style' => 'padding-top:5px;padding-left:15px;padding-right:15px;', 'role' => 'form']) !!}
        ...
        <table class="table table-striped table-font-sm">
          <thead class="thead-default thead-green">
              <tr>
                  ...
                  <th width="50%">Nama Gedung</th>
                  @if(in_array('building-CanEdit', $acc) || in_array('building-CanDelete', $acc))
          ...
          <tbody>
            ...
            foreach ($query as $data) {
              ...
              <tr>
                  ...
                  @if(in_array('building-CanEdit', $acc) || in_array('building-CanDelete', $acc))
                  <td align="center">
                      {!! Form::open(['url' => route('building.destroy', $data->Building_Id) , 'method' => 'delete', 'role' => 'form']) !!}
                      {{-- <a href="{{ url('master/building/'.$data->Building_Id.'?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Detail</a> --}}
                      @if(in_array('building-CanEdit', $acc))<a href="{{ url('master/building/'.$data->Building_Id.'/edit?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-info btn-sm">Edit</a>@endif
                      @if(in_array('building-CanDelete', $acc)){!! Form::button('Hapus', ['type'=>'submit','class'=>'btn btn-danger btn-sm hapus','data-id'=>$data->Building_Id]) !!}@endif
                      {!! Form::close() !!}
                  </td>
                  @endif
              </tr>
