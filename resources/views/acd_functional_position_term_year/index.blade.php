...
<section class="content">
  ...
  <div class="container-fluid-title">
    <div class="title-laporan">
      <h3 class="text-white">Jabatan Fungsional</h3>
    </div>
  </div>
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      ...
      <br>
          ...
          <div  class="row col-md-7">
                ...
                <a data-params="{{ $term_year }}"
                class="btn btn-success btn-sm" 
                id="kopidata"  
                href="javascript:">Copi Data</a>
          </div>
          </div>
          <br>
          <div class="row text-green" style="padding-top:5px;padding-left:15px;padding-right:15px;">
...
<div id="ubahpembayaran" class="w3-modal">
    ...
    <form id="frm-upload" action="{{ route('functional_position_term_year.store_kopidata') }}" method="POST" enctype="multipart/form-data">
        <div class="row text-green" style="padding-top:5px;padding-left:15px;padding-right:15px;">
          <div  class="row col-md-10">
            <label class="col-md-4">Semester Asal</label>
            <input type="text" name="term_from" id="term_from"  value="" readonly class="form-control form-control-sm col-md-7">
          </div>
          </div>
        <div class="row text-green" style="padding-top:5px;padding-left:15px;padding-right:15px;">
          <div  class="row col-md-10">
            <label class="col-md-4">Semester Tujuan</label>
            <select class="form-control form-control-sm col-md-7" name="term_dest" id="pilih">
              <option value="0">Pilih Tahun Ajaran</option>
              @foreach ( $select_term_year as $data )
                <option <?php if($term_year == $data->Term_Year_Id){ echo "selected"; } ?> value="{{ $data->Term_Year_Id }}">{{ $data->Term_Year_Name }}</option>
              @endforeach
  ...
  <script>
    $(document).on('click', '#kopidata', function (e) {
    document.getElementById("ubahpembayaran").style.display = "block";
    var term_now = $(this).data('params');

        $("[name='term_from']").val(term_now);
});
