  ...
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      <div class="panel-heading-green">
        <div class="pull-right tombol-gandeng dua">
          ...
          @if(in_array('country-CanAdd', $acc)) <a href="{{ url('mater/country/create?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Tambah data &nbsp;<i class="fa fa-plus"></i></a>@endif
        </div>
        <b>Negara</b>
      </div>
    </div>
    <br>
        <div class="bootstrap-admin-box-title right text-white">
          <!-- <b>Daftar Fakultas</b> -->
          {!! Form::open(['url' => route('country.index') , 'method' => 'GET', 'name' => 'form', 'class' => 'row text-green', 'style' => 'padding-top:5px;padding-left:15px;padding-right:15px;', 'role' => 'form']) !!}
        ...
        <table class="table table-striped table-font-sm">
          <thead class="thead-default thead-green">
              <tr>
                  ...
                  {{-- <th width="5%">No</th> --}}
                  <th width="15%">Kode Negara</th>
                  <th width="25%">Nama Negara</th>
                  <th width="20%">Akronim Negara</th>
                  <th width="15%">Nomor Urut</th>
                  @if(in_array('country-CanEdit', $acc) || in_array('country-CanDelete', $acc))
          ...
          <tbody>
            ...
            foreach ($query as $data) {
              ...
              <tr>
                  ...
                  <td>{{ $data->Country_Acronym }}</td>
                  <td>{{ $data->Order_Id }}</td>
                  @if(in_array('country-CanEdit', $acc) || in_array('country-CanDelete', $acc))
                  <td align="center">
                      {!! Form::open(['url' => route('country.destroy', $data->Country_Id) , 'method' => 'delete', 'role' => 'form']) !!}
                      {{-- <a href="{{ url('master/country/'.$data->Country_Id.'?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-success btn-sm">Detail</a> --}}
                      @if(in_array('country-CanEdit', $acc))<a href="{{ url('master/country/'.$data->Country_Id.'/edit?page='.$page.'&rowpage='.$rowpage.'&search='.$search) }}" class="btn btn-info btn-sm">Edit</a>@endif
                      @if(in_array('country-CanDelete', $acc)){!! Form::button('Hapus', ['type'=>'submit','class'=>'btn btn-danger btn-sm hapus','data-id'=>$data->Country_Id]) !!}@endif
                      {!! Form::close() !!}
                  </td>
                  @endif
              </tr>
